...
| `panels[].type` | `PanelSpec.type_` | mapped where dash9 has an equivalent (`timeseries`→`timeseries`, `gauge`→`gauge`, `bargauge`→`gauge` — Section H, radial vs. linear is only a visual distinction, meaningless in a terminal — `table`→`table`, `stat`→`stat`); anything else preserved per Section A, not executed |
...
| `fieldConfig.defaults.min`/`max` (`gauge`/`bargauge` panels only) | `ValidatedPanel.gauge_min`/`gauge_max` | `min` defaults to `0` when absent; `max` maps directly when present, or the `bool` unit's well-known implicit `0..1`, or `None` ("auto": the ceiling is the highest value across the panel's own live series, recomputed every refresh — real Grafana bargauge behavior for an export with no configured max, e.g. per-network-interface speed/MTU with no inherent bound) — never a fabricated number. A TOML-authored panel has no equivalent field (`SPEC.md` C.1); it always means a fixed `0..100`, unchanged from before this mapping existed |
